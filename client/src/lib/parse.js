...
function lcs(a, b) {
 ...
 export function parse(text) {
    ...
    parseEmoji(o);
    parseEmph(o);
    return `<p>${o[0].replace(/\\:/g, ":")}</p>${o[1]}`;
  }

  function parseDataURL(text) {
    if (text[0].substr(0, 10) === "data:image") {
      text[1] = `<img style="max-width: 50vw; max-height: 50vh;" src="${text[0]}">`;
      text[0] = "";
    }
  }
    ...
    for (let i = 0; i < a.length; i++) {
      if (a[i].match(/^https?:\/\/[^\s]+\.[^\s]+/)) {
        ...
        text[0] = text[0].replace(
          a[i],
          `<a href="${a[i].replace(/:/g, "\\:")}" target="_blank">${a[
            i
          ].replace(/:/g, "\\:")}</a>`
        );
      }
